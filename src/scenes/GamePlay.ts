...
import Projectile from "../gameComponents/projectiles/Projectile";
import EnemyPatientZero from "../gameComponents/enemy/EnemyPatientZero";
import EnemyProjectile from "../gameComponents/EnemyProjectiles/EnemyProjectile";
import Hud from "./Hud";
import EnemyXeroc from "../gameComponents/enemy/EnemyXeroc";
...
export default class GamePlay extends Phaser.Scene {
  ...
  private _projectileGroup: Phaser.GameObjects.Group;
  private _bonusGroup: Phaser.GameObjects.Group;
  private _enemyProjectileGroup: Phaser.GameObjects.Group;
  ...
  create() {
    ...
    this._projectileGroup = this.add.group({ runChildUpdate: true });
    this._enemyProjectileGroup = this.add.group({ runChildUpdate: true });
    ...
    new EnemyPatientZero({
      scene: this, x: 300, y: 100, key: "Patient_Zero"
    });
    ...
    this.physics.add.collider(this._projectileGroup, this._enemyGroup, this.hitEnemy, undefined, this);
    this.physics.add.collider(this._enemyProjectileGroup, this._player, this.projHitPlayer, undefined, this);
  ...
  addProjectile(projectile:Projectile) {
    this._projectileGroup.add(projectile);
  }

  addEnemyProjectile(projectile:EnemyProjectile) {
    this._enemyProjectileGroup.add(projectile);
  }
  ...
  hitEnemy(projectile: any, enemy: any) {
    ...
    const _projectile: Projectile = <Projectile>projectile;
  ...
  private iFrame:boolean = true;
  projHitPlayer(projectile: any, player: any){
    if (this.iFrame){
      this.iFrame = false;
      this.time.addEvent({
        delay: 1500, callback: () => {
      ...
      //this.events.emit("decrease-live");
      this.events.emit("set-life", [this.HP]);
      if(this.HP <= 0){
        this.iFrame = true;
        this.playerDeath()
        this.time.addEvent({
